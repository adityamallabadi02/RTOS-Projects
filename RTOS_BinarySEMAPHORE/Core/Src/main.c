...
osSemaphoreId BinarySemaHandle;
/* USER CODE BEGIN PV */
...
int main(void)
{
  ...
  /* definition and creation of BinarySema */
  osSemaphoreDef(BinarySema);
  BinarySemaHandle = osSemaphoreCreate(osSemaphore(BinarySema), 1);
  ...
  osThreadDef(Task_send, StartSendTask, osPriorityNormal, 0, 128);
  Task_sendHandle = osThreadCreate(osThread(Task_send), NULL);

  /* definition and creation of Task_receive */
  osThreadDef(Task_receive, ReceiveTask, osPriorityIdle, 0, 128);
...
void StartSendTask(void const * argument)
{
  ...
  for(;;)
  {
	  osSemaphoreRelease(BinarySemaHandle);
      osDelay(1000);
  }
  /* USER CODE END 5 */
}
...
void ReceiveTask(void const * argument)
{
  ...
  for(;;)
  {
	  osSemaphoreWait(BinarySemaHandle, osWaitForever);
